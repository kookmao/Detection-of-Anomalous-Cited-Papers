...
def visualize_anomalies(edges, anomalies, title='Graph with Anomalies'):
        G = nx.Graph()
        ...
        # Draw all edges
        nx.draw_networkx_edges(G, pos, edgelist=edges, edge_color='black', alpha=0.5)
        
        # Highlight anomalous edges
        nx.draw_networkx_edges(G, pos, edgelist=anomalies, edge_color='red', alpha=1.0, width=2.0)
...
class DynADModel(BertPreTrainedModel):
    ...
    ### added predicted_labels in order to capture the anomalous edges ###
    def evaluate(self, trues, preds):
        aucs = {}
        predicted_labels = []
        for snap in range(len(self.data['snap_test'])):
    ...
    def train_model(self, max_epoch):
        ...
        for epoch in range(max_epoch):
            ...
            if ((epoch + 1) % self.args.print_feq) == 0:
                ...
                aucs, auc_full, predicted_labels = self.evaluate(y_test, preds)

                for i in range(len(self.data['snap_test'])):
                    print("Snap: %02d | AUC: %.4f" % (self.data['snap_test'][i], aucs[i]))
                print('TOTAL AUC:{:.4f}'.format(auc_full))

                # Output and visualize anomalous edges
                for snap, labels in enumerate(predicted_labels):
                    anomalies = self.data['edges'][snap][labels == 1]
                    print(f"Snapshot {snap} Anomalous Edges: {anomalies}")
                    visualize_anomalies(self.data['edges'][snap], anomalies, title=f'Snapshot {snap} with Anomalies')
