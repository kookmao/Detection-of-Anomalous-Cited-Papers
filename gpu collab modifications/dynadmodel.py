...
class DynADModel(BertPreTrainedModel):
    ...
    def forward(self, init_pos_ids, hop_dis_ids, time_dis_ids, idx=None):
      # Move inputs to GPU
      init_pos_ids = init_pos_ids.to(self.args.device)
      hop_dis_ids = hop_dis_ids.to(self.args.device)
      time_dis_ids = time_dis_ids.to(self.args.device)
    ...
    def train_model(self, max_epoch):
        ...
        for epoch in range(max_epoch):
            ...
            for snap in self.data['snap_train']:
                ...
                # Move data to GPU
                int_embedding_pos = int_embeddings[snap].to(self.args.device)
                hop_embedding_pos = hop_embeddings[snap].to(self.args.device)
                time_embedding_pos = time_embeddings[snap].to(self.args.device)
                y_pos = self.data['y'][snap].float().to(self.args.device)
                
                int_embedding_neg = int_embeddings_neg[snap].to(self.args.device)
                hop_embedding_neg = hop_embeddings_neg[snap].to(self.args.device)
                time_embedding_neg = time_embeddings_neg[snap].to(self.args.device)
                y_neg = torch.ones(int_embedding_neg.size()[0]).to(self.args.device)
                ...
                loss_train += loss.detach().cpu().item()  # Move loss to CPU for logging
            ...
            if ((epoch + 1) % self.args.print_feq) == 0:
                ...
                for snap in self.data['snap_test']:
                    ...
                    int_embedding = int_embeddings[snap].to(self.args.device)
                    hop_embedding = hop_embeddings[snap].to(self.args.device)
                    time_embedding = time_embeddings[snap].to(self.args.device)
                    
                    with torch.no_grad():
                        output = self.forward(int_embedding, hop_embedding, time_embedding, None)
                        output = torch.sigmoid(output)
                    pred = output.squeeze().cpu().numpy()  # Move predictions back to CPU
                    preds.append(pred)
